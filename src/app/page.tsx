...
import {Calculator, ChevronRight, PanelsTopLeft, PiggyBank, Shield, TrendingUp} from 'lucide-react';
...
export default function Home() {
				...
				<div className={cn("flex", "flex-col", "gap-2")}>
					{
						card.subDescriptions.map((desc, index) => (
							<ul key={index} className={cn("flex", "items-center", "gap-2", "text-muted-foreground")}>
								{desc}
							</ul>
						))
					}
	...
	const footerDescriptorDetails: DescriptorDetails[] = [
		...
		},
		{
			...
			description: "Updated tax rates and calculations ensure you get the most accurate results."
		},
		{
			icon: <PanelsTopLeft className={iconClasses}/>,
			title: "User-Friendly Interface",
			description: "Clean, intuitive interface makes financial planning accessible to everyone."
		}
	]

	const FooterDescriptors = footerDescriptorDetails.map((desc) => (
		<div key={desc.title} className={cn("flex", "flex-col", "gap-2", "items-center", "text-center", "max-w-xs")}>
			<div className={iconBackgroundClasses}>
				{desc.icon}
			</div>
			<h4>{desc.title}</h4>
