...
const HeaderNavigation = () => {
	...
	const lgNavigationLinks = navigationLinkData.map(destination => (
		<NavigationMenuLink key={destination.href} href={destination.href}>
			<p>{destination.children}</p>
		</NavigationMenuLink>
	));

	const smNavigationLinks = navigationLinkData.map(destination => (
		<NavigationMenuItem key={destination.href}>
			<NavigationMenuLink
				href={destination.href!}
				className={cn('flex', 'flex-row', 'items-center', 'gap-3', 'w-full')}
			>
				<div className={cn('bg-primary/15', 'rounded-lg', 'p-3')}>{destination.icon}</div>
				{destination.children}
			</NavigationMenuLink>
		</NavigationMenuItem>
	));

	const lgScreenClassName = cn(['hidden', 'md:flex', 'flex-row', 'items-center', 'gap-4']);

	const smScreenClassName = cn(['flex', 'md:hidden', 'flex-col', 'items-start', 'gap-2']);
